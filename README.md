...
- Three color themes: **default** (synthwave/CRT), **cyberpunk-dark** (blue-black, red-orange, beveled), and **prisma** (positiv futurist, Blau/Cyan/Lime, Solarpunk with Aurora background, weiche Glows), selectable via `config.json`
...
| `theme` | string | `default` | Color theme. `default` is the original synthwave/CRT palette. `cyberpunk-dark` switches to a blue-black background with red-orange accents, beveled corners, and warm-toned icons. `prisma` replaces the CRT aesthetic with a positiv futurist, freundlich-neon Solarpunk design in Blau/Cyan/Lime: Aurora-gradient background (six drifting blobs), translucent panels with backdrop-blur, Outfit sans-serif typography, obere Neon-Kante, weiche Glows statt Scanlines, Breathe-Logo. Unknown values fall back to `default`. Alias: `skin`. |
